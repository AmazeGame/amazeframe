...
%%%-------------------------------------------------------------------
%%%
-module(test_process_handle).


-behaviour(gen_server).
-behaviour(ag_bizlogger_process_behaviours).
%% API
-export([start_link/1, write/2,make_process_name/2]).
...
%%% API
%%%===================================================================
-spec write(Name :: atom(), Bin :: binary()) -> ok.
write(ProcessName, InfoBin) ->
    ct:pal("test process write:~p~n", [{ProcessName, InfoBin}]).
...
start_link(#{biz:=Biz, index:=_} = Args) ->
    gen_server:start_link({local, Biz}, ?MODULE, [Args], []).
