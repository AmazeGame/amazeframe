...
%%%-------------------------------------------------------------------
%%%
-module(ag_debuglogger_flatlog_format).
...
format(Map = #{
    msg := {report, KeyVal}}, UsrConfig) when is_list(KeyVal) ->
    format(Map#{
...
format(Map = #{msg := {string, String}}, UsrConfig) ->
    format(Map#{
        msg := {report, #{report => unicode:characters_to_binary(String)}}},
        UsrConfig
    );
format(Map = #{msg := {Format, Terms}}, UsrConfig) ->
    ...
    NewFormat1 = agb_string:replace(NewFormat, "~p", "~500p"),
    format(Map#{
        msg := {report, #{
            report => unicode:characters_to_binary(io_lib:format(NewFormat1, Terms))}
        }},
        UsrConfig
    ).
        ...
        fun
            (K, V, Acc) when is_map(V) ->
                [format_msg(Parents ++ to_string(K, Config) ++ "_",
                    V, Config#{map_depth := Depth - 1}) | Acc];
            (_K, V, Acc) ->
                [Parents ++ to_string(V, Config), $\s | Acc]
        end,
        [],
        Data
    ).
