...
%%% @end
-module(ag_logger_flatlog_format).
...
format(Map = #{msg := {Format, Terms}}, UsrConfig) ->
    ...
    NewFormat= if RightWord == "~n" -> string:sub_string(Format,1,string:len(Format)-2); true-> Format end,
    NewFormat1 = agb_string:replace(NewFormat,"~p","~500p"),
    format(Map#{msg := {report,
                        #{unstructured_log =>
                          unicode:characters_to_binary(io_lib:format(NewFormat1, Terms))}}},
           UsrConfig).
