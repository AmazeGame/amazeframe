...
process_sharestring(ZipHandle) ->
	...
	case zip:zip_get(ShareStringFile, ZipHandle) of
		{error, Reason} ->
			agb_error:error("zip:zip_get ~p error:~p", [ShareStringFile, Reason]);
		{ok, ShareStrings} ->
...
process_relationhips(ZipHandle) ->
	...
	case zip:zip_get(RelationFile, ZipHandle) of
		{error, Reason} ->
			agb_error:error("zip:zip_get ~p error:~p", [RelationFile, Reason]);
		{ok, Relationships} ->
			...
			case xmerl_scan:string(binary_to_list(Binary)) of
				...
				% ExceptResult -> ErrorString = agb_string:sprintf("xmerl_scan:string error:~p", ExceptResult),
				% 	error(ErrorString)
			end
	end.
		...
		fun (_SheetInfo,{break,Context}) ->
			...
			(SheetInfo,{next_sheet,Context}) ->
				...
				case zip:zip_get(TargetFile, XlsxZipHandle) of
					{error, Reason} -> 
						agb_error:error("zip:zip_get ~p error:~p", [TargetFile, Reason]);
...
process_sheet(SheetBinary, SheetName ,RowHandler,InputContext) ->
	case xmerl_scan:string(binary_to_list(SheetBinary)) of
		{ParsedDocRootEl, _Rest} ->
			...
			Dimensions = ag_excel_util:xmlElement_from_name('dimension', XMLS),
			ColumnCount = ag_excel_util:get_column_count(ag_excel_util:xmlattribute_value_from_name('ref', Dimensions)),
			XmlSheetDatas = ag_excel_util:xmlElement_from_name('sheetData', XMLS),
			XmlRows = lists:filter(
				fun(XmlRow) ->
					case ag_excel_util:is_record(XmlRow, 'xmlElement') of
						false -> false;
		...
		% ExceptResult ->
		% 	ErrorString = agb_string:sprintf("xmerl_scan:string error:~p", ExceptResult),
		% 	error(ErrorString)
	end.
		...
		{ParsedDocRootEl, _Rest} ->
			...
			TextList = lists:map(
				fun(Node) ->
					...
					lists:foldl(
						fun(Element, AccTxt) ->
							case ag_excel_util:is_record(Element, xmlElement) of
								false -> AccTxt;
								true ->
									case Element#xmlElement.name of
										t -> get_element_text(Element);
										r ->
											TxtNode = ag_excel_util:xmlElement_from_name('t', Element#xmlElement.content),
...
get_element_text(Element) ->
	IsRecord = ag_excel_util:is_record(Element, xmlElement),
	if not IsRecord -> [];
		true ->
			if element(#xmlElement.name, Element) =:= 't' ->
				...
				TextNodes = lists:filter(
					fun(Text) ->
						ag_excel_util:is_record(Text, xmlText)
					end,
...
process_sheetinfos(ZipHandle) ->
	...
	case zip:zip_get(SheetFile, ZipHandle) of
		{error, Reason} ->
			agb_error:error("zip:zip_get ~p error:~p", [SheetFile, Reason]);
		{ok, WorkBook} ->
...
get_row(XmlRow, DefaultList) ->
	XmlCells = lists:filter(
		fun(XmlCell) ->
			case ag_excel_util:is_record(XmlCell, 'xmlElement') of
				false -> false;
	...
	case XmlCells of
		...
		[XmlCell1 | LeftXmlCells] ->
			...
			{X1, Y1} = ag_excel_util:get_field_number(CellName1),
			RowDataWithKey = ag_excel_util:take_nth_list(1, DefaultList, Y1),
			RowData1 = ag_excel_util:take_nth_list(X1 + 1, RowDataWithKey, CellValue1),
			lists:foldl(
				fun(Cell, RowData) ->
					{CellName, CellValue} = get_cell_info(Cell),
					{X, _Y} = ag_excel_util:get_field_number(CellName),
					ag_excel_util:take_nth_list(X + 1, RowData, CellValue)
		...
		SubNode ->
			case SubNode#xmlElement.content of
					   ...
					   XmlTexts ->
						   ...
						   ag_excel_util:str_normalize(lists:flatten(NewList))
				   end
	end.
...
get_cell_info(XmlCell) ->
	CellName = ag_excel_util:xmlattribute_value_from_name('r', XmlCell),
	CellValue = case ag_excel_util:has_attribute_value( 't', "s",XmlCell) of %% 1: t="s" => shareString | 2: t="str" => <v>String</v>
					true ->
						NewCellValue = list_to_integer(get_subnode_text('v',XmlCell)),
						get_xlsx_share_string(NewCellValue);
					false ->
						case ag_excel_util:has_attribute_value( 't', "b",XmlCell) of
