...
%%%-------------------------------------------------------------------
%%%
-module(ag_excel_util).

-include_lib("xmerl/include/xmerl.hrl").
%% API
-export([is_record/2]).
-export([xmlElement_from_name/2]).
-export([has_attribute_value/3]).
-export([xmlattribute_value_from_name/2]).
-export([get_column_count/1,get_column_string/1,get_field_number/1]).
-export([str_normalize/1]).
-export([to_list/1]).
-export([int_ceil/1]).
-export([escape/1]).
-export([take_nth_list/3]).
...
xmlattribute_value_from_name(Name, XmlNode) ->
	case lists:keyfind(Name,#xmlAttribute.name,XmlNode#xmlElement.attributes) of
		false-> agb_error:sprintf("xmlattribute_value_from_name exception: ~p ~p",[Name,XmlNode]);
...
xmlElement_from_name(Name,XmlNode)->
	case lists:keyfind(Name,#xmlElement.name,XmlNode) of
		false-> agb_error:sprintf("xmlElement_from_name exception: ~p ~p",[Name,XmlNode]);
		Element-> Element
	end.
