...
do_openzip_open(F, Options) ->
    ...
    {[#zip_comment{comment = C} | Files], In1} = get_central_dir(In0, fun raw_file_info_etc/5, Input),
    Z = zlib:open(),
...
do_openzip_get(#openzip{files = Files, in = In0, input = Input, output = Output, zlib = Z, cwd = CWD}) ->
    ZipOpts = #unzip_opts{output = Output, input = Input, file_filter = fun all/1,
        open_opts = [], feedback = fun silent/1, cwd = CWD},
...
do_openzip_get(F, #openzip{files = Files, in = In0, input = Input, output = Output, zlib = Z, cwd = CWD}) ->
...
openzip_list_dir(#openzip{zip_comment = Comment, files = Files}) ->
...
-spec(foldl(Fun, Acc0, Archive) ->
    {ok, Acc1} | {error, Reason} when
    Fun :: fun((FileInArchive, GetInfo, GetBin, AccIn) ->
        AccOut),
...
-spec(zip(Name, FileList, Options) ->
    RetValue when
    Name :: file:name(),
    FileList :: [FileSpec],
    FileSpec :: file:name() | {file:name(), binary()}
    | {file:name(), binary(), file:file_info()},
    Options :: [Option],
    Option :: memory | cooked | verbose | {comment, Comment} | {level, Level}
...
do_list_dir(F, Options) ->
    ...
    #list_dir_opts{input = Input, open_opts = OpO, raw_iterator = RawIterator} = Opts,
...
get_zip_opt([{level, Level} | Rest], Opts) ->           %% add by adrianx.lau@gmail.com
    get_zip_opt(Rest, Opts#zip_opts{level = Level});    %% for compress level
...
-spec(create(Name, FileList, Options) ->
    RetValue when
    Name :: file:name(),
    FileList :: [FileSpec],
    FileSpec :: file:name() | {file:name(), binary()}
...
put_z_files([], _Z, Out, Pos, _Opts, Acc) -> {Out, lists:reverse(Acc, []), Pos};
put_z_files([F | Rest], Z, Out0, Pos0,
    #zip_opts{input = Input, output = Output, open_opts = OpO,
        feedback = FB, cwd = CWD, level = CompressLevel} = Opts, Acc) ->
    In0 = [],
    F1 = add_cwd(CWD, F),
    FileInfo = Input({file_info, F1}, In0),
    Type = FileInfo#file_info.type,
    UncompSize = get_un_comp_size(Type, FileInfo),
    ...
    TMap = #{
        method => CompMethod, uncomp_size => UncompSize, out2 => Out2, zip_f1 => F1, pos => 0, input => Input,
        output => Output, op0 => OpO, zip_z => Z, compress_level => CompressLevel, type => Type
    },
    {Out3, CompSize, CRC} = put_z_file(TMap, CompMethod, UncompSize, Type),
    FB(FileName),
    ...
    TMap1 = #{
        type => Type, out5 => Out5, this_acc => ThisAcc, pos1 => Pos2, input => Input, zip_f1 => F1, zip_f => F,
        zip_z => Z, opts => Opts
    },
    {Out6, SubAcc, Pos3} = get_out_and_sub_and_pos(TMap1),
    Acc2 = lists:reverse(SubAcc) ++ Acc,
    put_z_files(Rest, Z, Out6, Pos3, Opts, Acc2).

get_un_comp_size(Type, FileInfo) ->
    case Type of
        regular ->
...
%% compress a file
%%put_z_file(_Method, Sz, Out, _F, Pos, _Input, _Output, _OpO, _Z, _Level, directory) ->
%%    {Out, Pos + Sz, 0};
%%put_z_file(_Method, 0, Out, _F, Pos, _Input, _Output, _OpO, _Z, _Level, regular) ->
%%    {Out, Pos, 0};
%%put_z_file(?STORED, UncompSize, Out0, F, Pos0, Input, Output, OpO, Z, _Level, regular) ->
%%    In0 = [],
...
%%put_z_file(?DEFLATED, UncompSize, Out0, F, Pos0, Input, Output, OpO, Z, Level, regular) ->
%%    In0 = [],
%%    In1 = Input({open, F, OpO -- [write]}, In0),
...
%% compress a file
put_z_file(TMap, _CompMethod, Sz, directory) ->
    Out = maps:get(out2, TMap),
    Pos = maps:get(pos, TMap),
    {Out, Pos + Sz, 0};
put_z_file(TMap, _CompMethod, 0, regular) ->
    Out = maps:get(out2, TMap),
    Pos = maps:get(pos, TMap),
    {Out, Pos, 0};
put_z_file(TMap, ?STORED, UncompSize, regular) ->
    #{
        uncomp_size := UncompSize, out2 := Out0, zip_f1 := F, pos := Pos0, input := Input,
        output := Output, op0 := OpO, zip_z := Z
    } = TMap,
    In0 = [],
    In1 = Input({open, F, OpO -- [write]}, In0),
    CRC0 = zlib:crc32(Z, <<>>),
    ...
    Input(close, In2),
    {Out1, Pos0 + erlang:iolist_size(Data), CRC};
put_z_file(TMap, ?DEFLATED, UncompSize, regular) ->
    #{
        uncomp_size := UncompSize, out2 := Out0, zip_f1 := F, pos := Pos0, input := Input,
        output := Output, op0 := OpO, zip_z := Z, compress_level := Level
    } = TMap,
    In0 = [],
    In1 = Input({open, F, OpO -- [write]}, In0),
    ok = zlib:deflateInit(Z, Level, deflated, -?MAX_WBITS, 8, default),
...
server_loop(Parent, OpenZip) ->
    receive
        {From, {open, Archive, Options}} ->
            do_server_loop(Archive, Options, From, Parent);
        {From, close} ->
...
-spec(zip_open(Archive) ->
    {ok, ZipHandle} | {error, Reason} when
    Archive :: file:name() | binary(),
    ZipHandle :: handle(),
    Reason :: term()).
zip_open(Archive) ->
...
-spec(zip_close(ZipHandle) ->
    ok | {error, einval} when
    ZipHandle :: handle()).
zip_close(Pid) when is_pid(Pid) ->
    request(self(), Pid, close).

-spec(zip_get(FileName, ZipHandle) ->
    {ok, Result} | {error, Reason} when
    FileName :: file:name(),
    ZipHandle :: handle(),
    Result :: file:name() | {file:name(), binary()},
...
openzip_tt(#openzip{zip_comment = ZipComment, files = Files}) ->
    ...
    lists_foreach(
        fun({#zip_file{comp_size = CompSize,
            name = FileName,
            comment = FileComment,
            info = FI}, _}) ->
...
get_cd_loop(N, In0, RawIterator, Input, Acc0) ->
    ...
    BCD =
        case B of
            <<?CENTRAL_FILE_MAGIC:32/little, XBCD/binary>> ->
                XBCD;
            _ ->
    ...
    {FileName, Comment, BExtra} = get_name_extra_comment(B2, FileNameLen, ExtraLen, CommentLen),
...
find_eocd_header(<<?END_OF_CENTRAL_DIR_MAGIC:32/little, Rest/binary>>) ->
    Rest;
find_eocd_header(<<_:8, Rest/binary>>) when byte_size(Rest) > ?END_OF_CENTRAL_DIR_SZ - 4 ->
...
raw_file_info_public(CD, FileName, FileComment, BExtraField, Acc0) ->
    ...
    H2 =
        case H1 of
            {ZF, Extra} when is_record(Extra, zip_file_extra) ->
                ZF;
            Other ->
                Other
        end,
    ...
    case Input({read, ?LOCAL_FILE_HEADER_SZ}, In0) of
        ...
        {<<?LOCAL_FILE_MAGIC:32/little, B/binary>>, In1} ->
            ...
            {CompSize, CRC32} = case GPFlag band 8 =:= 8 of
                                    true ->
                                        {ZipFile#zip_file.comp_size,
                                            Extra#zip_file_extra.crc32};
                                    false ->
                                        {LH#local_file_header.comp_size,
...
%% compress zlib support Level option
-spec compress(Data, Level) ->
    Compressed when
    Data :: iodata(),
    Level :: integer() | default,
    Compressed :: binary().
...
-spec pack(Zip :: file:filename(),
    InputDir :: file:dirname(),
    Config :: [{wildcard, [file:name_all()]}]) ->
    {ok, {file:filename(), binary()}} |{error, term()}.
pack(Zip, InputDir, Config) ->
    Wildcard = proplists:get_value(wildcard, Config),
    Files = lists:flatmap(fun(W) -> filelib:wildcard(W) end, Wildcard),
    InputDirAbs = filename:absname(InputDir),
    InputDirAbs2 =
        case lists:reverse(InputDirAbs) of
            [$/ | _RDir] ->
                InputDirAbs;
            InputDirAbs0 ->
                lists:reverse([$/ | InputDirAbs0])
        end,
    ...
    ZFiles =
        lists:map(
            fun(File) ->
                ...
                {ok, Binary} = file:read_file(AbsFile),
                {AbsFile -- InputDirAbs2, Binary}
            end,
            Files
        ),
    agb_zip:create(Zip, ZFiles, [{level, 9}]).
