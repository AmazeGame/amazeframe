...
%% API
%%====================================================================
-spec from_string(CodeStr :: string(), CompileFormsOptions :: list()) ->
    {atom(), binary()} | no_return().
from_string(CodeStr, CompileFormsOptions) ->
    do_from_string(CodeStr ++ " ", CompileFormsOptions).
...
%%--------------------------------------------------------------------
-spec from_string(CodeStr :: string()) ->
    {atom(), binary()} | no_return().
from_string(CodeStr) ->
    do_from_string(CodeStr ++ " ", []).
...
do_from_string(CodeStr, CompileFormsOptions) ->
    ...
    %%    When encountering an -include or -include_dir directive,
    ...
    IncludeSearchPath = ["." | lists:reverse([Dir || {i, Dir} <- CompileFormsOptions])],
    {RevForms, _OutMacroDict} = scan_and_parse(CodeStr, Filename, 1, [], InitMD, IncludeSearchPath),
    Forms = lists:reverse(RevForms),
...
scan_and_parse(RemainingText, CurrFilename, CurrLine, RevForms, MacroDict, IncludeSearchPath) ->
    case scanner(RemainingText, CurrLine, MacroDict) of
        ...
        {include, NLine, NRemainingText, IncludeFilename} ->
            ...
            %%?PRINT("include file ~p contents: ~n~p~nRemainingText = ~p~n",
            %% [IncludeFilename,IncludeFileRemainingTextents, RemainingText]),
            ...
            {RevIncludeForms, IncludedMacroDict} = scan_and_parse(IncludeFileRemainingTextents, IncludeFilename, 1, [],
                IncludeMacroDict, IncludeSearchPath),
            %?PRINT("include file results = ~p~n", [R]),
            ...
            scan_and_parse(NRemainingText, CurrFilename, NLine, RevIncludeForms ++ RevForms,
...
is_only_comments([], _) ->
    true;
is_only_comments([ 32 | T], not_in_comment) ->
...
is_only_comments([$\n | T], not_in_comment) ->
    is_only_comments(T, not_in_comment); % skipping whitspace outside of comment
is_only_comments([$% | T], not_in_comment) ->
    is_only_comments(T, in_comment);     % found start of a comment
is_only_comments(_, not_in_comment) ->
    false;
% found any significant char NOT in a comment
is_only_comments([$\n | T], in_comment) ->
    is_only_comments(T, not_in_comment); % found end of a comment
is_only_comments([_ | T], in_comment) ->
    is_only_comments(T, in_comment).     % skipping over in-comment chars
...
macro_params_body_def([{')', _}, {',', _} | Toks], RevParams) ->
    {lists:reverse(RevParams), macro_body_def(Toks, [])};
...
subst_macros_subst_args_for_vars({[Param | Params], BodyToks}, [Arg | Args]) ->
    NBodyToks = lists:keyreplace(Param, 3, BodyToks, {var, 1, Arg}),
    subst_macros_subst_args_for_vars({Params, NBodyToks}, Args).

read_include_file(Filename, IncludeSearchPath) ->
    ct:pal("IncludeSearchPath :~p", [IncludeSearchPath]),
    ct:pal("IncludeSearchPath Filename :~p", [Filename]),
