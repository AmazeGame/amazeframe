...
%%
%% @spec integer_to_base58(integer()) ->  base58()

-spec from_integer(integer()) ->
    base58().
from_integer(0) ->
    [];
...
-spec to_integer(char(), base58()) ->
    'error' | integer().
to_integer(C, []) ->
    C;
to_integer(C, [X | Xs]) ->
    to_integer(C * 58 + charb58(X), Xs).
...
-spec to_binary(base58()) ->
    binary().
to_binary(Base58) ->
    Bin = binary:encode_unsigned(to_integer(Base58)),
...
%%
%% @spec binary_to_base58(binary()) ->  base58()

-spec from_binary(binary()) ->
    base58().
from_binary(Binary) when is_binary(Binary) ->
    Base58 = from_integer(binary:decode_unsigned(Binary)),
